#!/bin/bash
# Script para atualizar a configuração do Claude Desktop para usar o analisador de projeto
...
echo "Atualizando configuração do Claude Desktop para usar o analisador de projeto..."
TMP_FILE=$(mktemp)

# Atualizar o servidor Continuity Protocol para usar o analisador
jq --arg cmd "python3" --arg arg "${CONTINUITY_DIR}/simple-project-analyzer.py" --arg desc "Continuity Protocol - Analisador de Projeto" '
...
if jq empty "$TMP_FILE" 2>/dev/null; then
    ...
    echo "O servidor Continuity Protocol agora usa: simple-project-analyzer.py"
