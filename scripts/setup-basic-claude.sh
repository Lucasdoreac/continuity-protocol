#!/bin/bash
# Script para configurar a integração do servidor básico com Claude Desktop
...
SERVER_NAME="basic-continuity-protocol"
...
echo "Configurando servidor MCP básico no Claude Desktop..."
TMP_FILE=$(mktemp)

# Ler configuração atual e adicionar/atualizar o servidor
jq --arg name "$SERVER_NAME" --arg cmd "python3" --arg arg "${CONTINUITY_DIR}/basic-mcp-server.py" '
.mcpServers[$name] = {
  "command": $cmd,
  "args": [$arg],
  "description": "Basic Continuity Protocol - Servidor MCP Mínimo"
}
...
if jq empty "$TMP_FILE" 2>/dev/null; then
    ...
    echo -e "${GREEN}Servidor MCP básico configurado com sucesso no Claude Desktop!${NC}"
    echo "Nome: $SERVER_NAME"
    echo "Comando: python3 ${CONTINUITY_DIR}/basic-mcp-server.py"
else
