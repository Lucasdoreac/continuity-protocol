#!/bin/bash
# Script para configurar a integração do servidor simplificado com Claude Desktop
...
SERVER_NAME="simple-continuity-protocol"
...
echo "Configurando servidor MCP simplificado no Claude Desktop..."
TMP_FILE=$(mktemp)

# Ler configuração atual e adicionar/atualizar o servidor
jq --arg name "$SERVER_NAME" --arg cmd "python3" --arg arg "${CONTINUITY_DIR}/src/servers/simple-mcp-server.py" '
.mcpServers[$name] = {
  "command": $cmd,
  "args": [$arg],
  "description": "Simple Continuity Protocol - Servidor MCP Básico"
}
...
if jq empty "$TMP_FILE" 2>/dev/null; then
    ...
    echo -e "${GREEN}Servidor MCP simplificado configurado com sucesso no Claude Desktop!${NC}"
    echo "Nome: $SERVER_NAME"
    echo "Comando: python3 ${CONTINUITY_DIR}/src/servers/simple-mcp-server.py"
else
