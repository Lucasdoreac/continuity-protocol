#!/bin/bash
# Script para atualizar a configuração do Claude Desktop para usar o servidor básico unificado
...
echo "Atualizando configuração do Claude Desktop para usar o servidor básico unificado..."
TMP_FILE=$(mktemp)

# Atualizar o servidor Continuity Protocol para usar o servidor básico unificado
jq --arg cmd "python3" --arg arg "${CONTINUITY_DIR}/basic-unified-server.py" --arg desc "Continuity Protocol - Servidor Básico Unificado" '
...
if jq empty "$TMP_FILE" 2>/dev/null; then
    ...
    echo "O servidor Continuity Protocol agora usa: basic-unified-server.py"
