#!/bin/bash
# Script para iniciar o servidor unificado Continuity Protocol

CONTINUITY_DIR="/Users/lucascardoso/apps/MCP/CONTINUITY"
SERVER_SCRIPT="${CONTINUITY_DIR}/unified-continuity-server.py"
LOG_FILE="${CONTINUITY_DIR}/logs/continuity-server.log"
...
if [ -f "$PID_FILE" ]; then
    ...
    if ps -p "$PID" > /dev/null; then
        ...
        echo "Use: ${CONTINUITY_DIR}/stop-continuity-server.sh para parar"
        exit 1
    else
...
if ps -p "$PID" > /dev/null; then
    ...
    echo "PID: $PID"
    echo "Log: $LOG_FILE"
    echo "Para parar o servidor: ${CONTINUITY_DIR}/stop-continuity-server.sh"
else
